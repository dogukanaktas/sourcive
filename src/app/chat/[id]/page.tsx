...
export default function ChatPage() {
  ...
  const { messages, input, isLoading, error, usage, isContextLong, handleInputChange, handleSubmit, setInput, clearMessages, resetWithMessages, stop } =
    useChat();
  ...
  // Save new messages to DB after streaming completes
  useEffect(() => {
    if (!isLoading && dbLoaded && messages.length > savedCountRef.current) {
      const unsaved = messages.slice(savedCountRef.current);
      // Don't save if last assistant message is still empty (error)
  ...
  const onFormSubmit = useCallback(async (e: React.FormEvent) => {
    if (!conversationCreated.current && input.trim()) {
      conversationCreated.current = true;
      await createConversation(conversationId, input.trim());
    }
    handleSubmit(e);
  }, [conversationId, createConversation, handleSubmit, input]);

  const isEmpty = messages.length === 0 && dbLoaded;
  ...
  return (
    <div className="flex h-full flex-col bg-gradient-to-b from-background to-muted/20">
      ...
      <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur px-6 py-3 shrink-0 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity pl-8">
